 ...
 *
 * NOTE: This function only works on the client-side (browser environment).
 * On the server (Vercel), it returns the HTML as-is without sanitization.
 * This is safe because:
 * 1. The HTML is generated by our own code (summaryFormatter.ts)
 * 2. User input is already escaped in summaryFormatter.ts using escapeHtml()
 * 3. The sanitization still happens on the client when the HTML is rendered
 *
...
export function sanitizeHtml(html: string | undefined | null): string {
  ...
  // Server-side: skip sanitization (HTML is already escaped in summaryFormatter)
